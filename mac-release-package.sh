...
PACKAGE=NUK-client-${VERSION}
echo PACKAGE="${PACKAGE}"
CHANGELOG=NUK-client-${VERSION}.changelog.txt
OBFUSCATE=$2
MACVERSION=$3
if [ -x ${MACVERSION} ];
then
...
FILES="${FILES} NukaCoin.exe NukaCoinservice.exe"
...
echo copy resources
cp installer/lib/JavaExe.exe NukaCoin.exe
cp installer/lib/JavaExe.exe NukaCoinservice.exe
cp -a ${FILES} nxt
